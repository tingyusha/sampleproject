...
class UITest_Service: XCTestCase {
    ...
    func test00_drugService() {
        ...
        app.tables.elementAtIndex(0).childrenMatchingType(.Cell).elementAtIndex(1).tap()
        app.tables.childrenMatchingType(.Cell).elementAtIndex(1).tap()
        sleep(1)
        app.navigationBars.childrenMatchingType(.Button).matchingIdentifier("Back").elementAtIndex(0).tap()
